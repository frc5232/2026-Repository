...
public class Vision extends SubsystemBase {
  ...
  public void calculatePose(){
    ...
    for(int i = 0; i < listOfCameras.size();i++){
      ...
      if(camerasHaveTargets.get(i) == true){
        ...
        if(aprilTagFieldLayouts.getTagPose(m.getLatestResult().getBestTarget().getFiducialId()).isPresent()){
          ...
          currentPose3d = PhotonUtils.estimateFieldToRobotAprilTag(m.getLatestResult().getBestTarget().getBestCameraToTarget(), aprilTagFieldLayouts.getTagPose(m.getLatestResult().getBestTarget().getFiducialId()).get(), robotToCam.get(i));
        }
        //sets ouir 
    ...
    //updating our current pose with our total values then dividing by amount of cameras and checking for anomilies
    if(size != 0){
  ...
  /**
   * 
   * @param x Our Array list of cameras which we iterate through and set another list to true or false based off of whether or not that camera has a target
   *if it doesnt have a target sets the other Array list to False
   */
  private void checkallCameras(ArrayList<PhotonCamera> x){
    // iterating through the cameras and checking if they have targets if they do updating it to true else make it false
  ...
  private boolean yawFixer(Pose3d pp){
    ...
    direction =pp.getRotation().getMeasureZ().compareTo(lastPose3d.getRotation().getMeasureZ());
      ...
      if(pastdir == headingDir){
        ...
        if(pp.getRotation().getMeasureZ().isNear(lastPose3d.getRotation().getMeasureZ(), 0.12)){
          return true;
        }
   ...
   * @return whether or whether not to trust the pose if yes then return the pose we passed else return past pose
   */
